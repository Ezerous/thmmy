...
const baseUnreadURL = 'https://www.thmmy.gr/smf/index.php?action=unread;all;theme=4';

exports.getUnreadPosts = async function(cookieJar, options) {
    options = assignOptions(options);

    const boards = options.boards;
    let unreadURL = baseUnreadURL;
    ...
    const pagesToFetch = Math.ceil(options.unreadLimit/20); //There are 20 unread posts per page
    ...
    function fetchUnreadPage(unreadPageURL) {
        return new Promise(function(resolve, reject) {
            rp({url: unreadPageURL, jar: cookieJar})
                .then(function(html) {
                    return resolve(extractUnread(html));
                })
    ...
    function fetchUnreadPagesURLs() {
        return new Promise(function(resolve, reject) {
            rp({url: unreadURL, jar: cookieJar})
                .then(function(html) {
                    return resolve(extractNavPages(html));
                })
                .catch(function(error) {
                    return reject(error);
                });
    ...
    function extractNavPages(html) {
        ...
        try {
            ...
            if (lastPageElement.length>0){
                let lastPage = (/start=(\d+)/i.exec(lastPageElement.attr('href'))[1])/20 + 1;

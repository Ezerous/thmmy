 ...
 */
exports.forumTimeToTimestamp = function(forumTime) {
    if (forumTime) {
        if (/\D+\s\D+/i.test(forumTime)) {
            //e.g. Today at 23:12:64 or Σήμερα στις 23:12:64
            const today = moment().format('MMMM DD, Y');
            const time = today + ', ' + /\S+\s\S+\s(\d+:\d+:\d+)/i.exec(forumTime)[1] + ' +0200';
            return moment(time, 'MMMM DD, Y, HH:mm:ss ZZ').unix();
        } else {
            //e.g. October 30, 2017, 14:26:51
            const time = /(.*\d+)/i.exec(forumTime)[1] + ' +0200';
            let momentTime = moment(time, 'MMMM DD, Y, HH:mm:ss ZZ').unix();
            if (momentTime)  //Success for 'en' locale
                return momentTime;
            else {   //Attempting conversion for 'el' locale
                return moment(time, 'MMMM DD, Y, HH:mm:ss ZZ', 'el').unix();
            }
        }
    }
};
